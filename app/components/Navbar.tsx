...
const Navbar: FC<NavbarProps> = ({ balance = 0, showBalance = true, isLoggedIn = false }) => {
  return (
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          <div class="flex items-center">
            <i data-lucide="hexagon" class="text-brand w-8 h-8"></i>
            {/* Tautan logo diarahkan ke Dashboard jika sudah login, jika belum ke Frontpage */}
            <a href={isLoggedIn ? "/dashboard" : "/"} class="ml-2 font-bold text-xl tracking-tight text-gray-900 dark:text-white hover:text-brand transition-colors">
              SMMPedia
            </a>
          </div>
          <div class="flex items-center space-x-4">
            
            {/* SALDO HANYA MUNCUL JIKA showBalance TRUE dan USER SUDAH LOGIN */}
            ...
            {/* LOGIKA KONDISIONAL TOMBOL LOGIN / LOGOUT */}
            {isLoggedIn ? (
              ...
              <a href="/login" class="flex items-center px-4 py-2 bg-brand text-white text-sm font-bold rounded-lg hover:opacity-90 transition-opacity">
