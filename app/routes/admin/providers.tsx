      ...
      <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm h-fit">
            <h2 class="font-bold mb-4 border-b pb-2">Tambah / Edit Provider</h2>
            <form method="POST" class="space-y-4 text-sm">
              <div>
                <label class="block font-semibold mb-1">Slug (ID Unik Huruf Kecil)</label>
                <input type="text" name="slug" required placeholder="contoh: buzzerpanel" class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2" />
              </div>
              <div>
              ...
              <div>
                <label class="block font-semibold mb-1 text-red-600">API Key</label>
                <input type="text" name="api_key" placeholder="Wajib untuk BuzzerPanel / Provider Lain" class="w-full bg-gray-50 border border-red-200 rounded-lg p-2" />
              </div>
              <div>
                <label class="block font-semibold mb-1 text-red-600">Secret Key</label>
                <input type="text" name="secret_key" placeholder="Wajib untuk BuzzerPanel" class="w-full bg-gray-50 border border-red-200 rounded-lg p-2" />
              </div>
              <div class="mt-4 pt-4 border-t">
                <p class="text-xs text-gray-500 mb-2 font-semibold">Khusus Custom Provider (BuzzerPanel Kosongkan):</p>
                <label class="block font-semibold mb-1">JSON Order Body</label>
                <textarea name="order_body_template" rows={2} class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 font-mono text-xs"></textarea>
              </div>
              <div>
                <label class="block font-semibold mb-1">JSON Status Body</label>
                <textarea name="check_body_template" rows={2} class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 font-mono text-xs"></textarea>
              </div>
              <div>
                <label class="block font-semibold mb-1">Response Mapping JSON</label>
                <textarea name="response_mapping" rows={2} class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 font-mono text-xs"></textarea>
              </div>
              <button type="submit" class="w-full bg-brand text-white font-bold py-2 rounded-lg">Simpan Konfigurasi</button>
            </form>
          </div>
                ...
                {providers.map((p: any) => (
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-center">
                      <span class={`px-2 py-1 rounded-full text-[10px] font-bold ${p.api_key ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
