...
const routeHandler = async (c: any) => {
  ...
  const servicesData = await c.env.DB.prepare(`
    ...
    FROM services s JOIN categories c ON s.category_id = c.id WHERE s.status = 'active'
  `).all()
    ...
    <MemberLayout title="Pesanan Baru" balance={balance}>
      <div class="p-6 md:p-8 max-w-5xl mx-auto">
        ...
        {categories.map((category) => (
          <div class="mb-10">
            <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200 flex items-center"><i data-lucide="tag" class="w-5 h-5 mr-2 text-brand"></i>{category}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {services.filter(s => s.category === category).map((srv: any) => (
                <div class="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col justify-between h-full">
                  ...
                  <button onclick={`openOrderModal('${srv.id}', '${srv.name}', ${srv.rate + (srv.margin || 0)}, ${srv.min_order}, ${srv.max_order})`} class="mt-4 w-full py-2 bg-blue-50 text-brand rounded-lg font-medium hover:bg-brand hover:text-white transition text-sm">Pesan</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div id="orderModal" class="hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 w-full max-w-md overflow-hidden">
            <div class="p-4 border-b border-gray-200 flex justify-between items-center">
              ...
              <button onclick="document.getElementById('orderModal').classList.add('hidden')" class="text-gray-500 hover:text-gray-800"><i data-lucide="x" class="w-6 h-6"></i></button>
            </div>
            ...
            <form id="orderForm" class="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" id="targetLink" required class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-brand" />
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-semibold mb-1">Jumlah</label>
                  <input type="number" id="orderQuantity" required class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-brand" />
                  <p class="text-xs text-gray-500 mt-1" id="modalMinMax"></p>
                </div>
                <div>
                  <label class="block text-sm font-semibold mb-1">Total Biaya</label>
                  <input type="text" id="totalCost" readonly value="Rp 0" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-2.5 font-bold cursor-not-allowed" />
                </div>
              </div>

              <button type="submit" id="submitOrderBtn" class="w-full bg-brand text-white font-bold p-3 rounded-lg hover:opacity-90 mt-4">Bayar & Proses Pesanan</button>
            </form>
          </div>
        </div>
